...
import { FaEye, FaEdit, FaTrash, FaPlus, FaTimes, FaWarehouse } from "react-icons/fa";

const API_URL = "http://localhost:5000/almacenes";

// Columnas para la vista previa en la tabla
const columnasVistaPrevia = [
  { key: "id_almacen", label: "ID", width: "80px", centrado: true },
  { key: "nombre", label: "Nombre", width: "30%" },
  { key: "direccion", label: "Dirección", width: "30%" },
  { key: "encargado", label: "Encargado", width: "20%" },
  { key: "activo", label: "Estado", width: "20%", centrado: true },
];

// Todas las columnas para el detalle completo
const todasLasColumnas = [
  { key: "id_almacen", label: "ID Almacén" },
...
const ejemplosPlaceholder = {
  nombre: "Ej: Almacén Principal",
  direccion: "Ej: Av. Industrial 123",
...
export default function TablaAlmacenes() {
  ...
  useEffect(() => {
    if (almacenes.length > 0) {
      const maxId = Math.max(...almacenes.map(item => item.id_almacen || 0));
      setNextId(maxId + 1);
    } else {
      setNextId(1);
    }
    ...
    try {
      ...
      switch (action) {
        case "create":
          ...
          break;
        case "update":
          response = await axios.put(`${API_URL}/${data.id_almacen}`, data);
          break;
        case "delete":
          await axios.delete(`${API_URL}/${data.id_almacen}`);
          break;
        default:
  ...
  const datosFiltrados = almacenes.filter((item) =>
  ...
  const eliminarElemento = async (id) => {
    if (window.confirm(`¿Está seguro que desea eliminar este almacén?`)) {
      try {
        await handleCRUD("delete", { id_almacen: id });
      } catch (err) {
  ...
  const getInputWidth = (key) => {
    if (key === 'id_almacen') return 'w-24';
    if (key === 'telefono') return 'w-32';
    return 'w-full';
  };
  ...
  if (loading && almacenes.length === 0) {
    return (
      <div className="flex justify-center items-center h-64">
        ...
        <span className="ml-4">Cargando almacenes...</span>
      </div>
    );
  }
    ...
    <div className="w-full border border-gray-400 rounded shadow-lg">
        ...
        <div className="bg-yellow-100 p-4 border-b border-gray-300">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-lg font-semibold">
              {nuevoElemento ? `Nuevo almacén` : `Editando almacén`}
            ...
            {todasLasColumnas.map((col) => (
              <div key={`form-${col.key}`} className="mb-1">
                ...
                {col.key === 'activo' ? (
                  ...
                  <input
                    type={col.key === 'telefono' ? 'tel' : 'text'}
                    name={col.key}
                    ...
                    disabled={(col.key === 'id_almacen') || loading}
                    readOnly={col.key === 'id_almacen'}
                  />
                )}
              </div>
            ))}
      ...
      {viendo && (
        <div className="bg-blue-50 p-4 border-b border-gray-300">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-lg font-semibold">Detalle Completo del Almacén</h3>
            <div className="flex gap-2">
              ...
              <button
                onClick={() => eliminarElemento(viendo.id_almacen)}
        ...
        <table className="min-w-full border-collapse bg-white" style={{ tableLayout: "fixed" }}>
          ...
          <tbody>
            {datosFiltrados.map((item) => (
              <tr key={`row-${item.id_almacen}`} className="hover:bg-gray-200">
                {columnasVistaPrevia.map((col) => (
                  <td
                    key={`cell-${item.id_almacen}-${col.key}`}
                    className={`p-2 border border-gray-300 ${
                      col.centrado ? "text-center" : "text-left"
                    }`}
