...
const ejemplosPlaceholder = {
  nombre: "Ej: Distribuidora S.A.",
  telefono: "Ej: 555-1234567",
  direccion: "Ej: Av. Principal #123",
  correo: "Ej: contacto@distribuidora.com",
};
...
export default function TablaProveedores() {
  ...
  useEffect(() => {
    if (proveedores.length > 0) {
      const maxId = Math.max(...proveedores.map(item => item.id_proveedor || 0));
      setNextId(maxId + 1);
    } else {
      setNextId(1);
    }
    ...
    try {
      ...
      switch (action) {
        case "create":
          ...
          break;
        case "update":
          response = await axios.put(`${API_URL}/${data.id_proveedor}`, data);
          break;
        case "delete":
          await axios.delete(`${API_URL}/${data.id_proveedor}`);
          break;
        default:
  ...
  const datosFiltrados = proveedores.filter((item) =>
  ...
  const eliminarElemento = async (id) => {
    if (window.confirm(`¿Está seguro que desea eliminar este proveedor?`)) {
      try {
        await handleCRUD("delete", { id_proveedor: id });
      } catch (err) {
  ...
  const getInputWidth = (key) => {
    if (key === 'id_proveedor') return 'w-24';
    return 'w-full';
  };
    ...
    <div className="w-full border border-gray-400 rounded shadow-lg">
        ...
        <div className="bg-yellow-100 p-4 border-b border-gray-300">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-lg font-semibold">
              {nuevoElemento ? `Nuevo proveedor` : `Editando proveedor`}
              ...
              <div key={`form-${col.key}`} className="mb-1">
                ...
                {col.key === 'activo' ? (
                  ...
                  <input
                    ...
                    disabled={(col.key === 'id_proveedor') || loading}
                    readOnly={col.key === 'id_proveedor'}
                  />
                )}
              </div>
            ))}
      ...
      {viendo && (
        <div className="bg-blue-50 p-4 border-b border-gray-300">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-lg font-semibold">Detalle Completo del Proveedor</h3>
            <div className="flex gap-2">
              ...
              <button
                onClick={() => eliminarElemento(viendo.id_proveedor)}
        ...
        <table className="min-w-full border-collapse bg-white" style={{ tableLayout: "fixed" }}>
          ...
          <tbody>
            {datosFiltrados.map((item) => (
              <tr key={`row-${item.id_proveedor}`} className="hover:bg-gray-200">
                {columnasVistaPrevia.map((col) => (
                  <td
                    key={`cell-${item.id_proveedor}-${col.key}`}
                    className={`p-2 border border-gray-300 ${
                      col.centrado ? "text-center" : "text-left"
                    }`}
